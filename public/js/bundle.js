...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    //all these properties here belongs to mapbox website
    container: 'map',
  ...
  var bounds = new mapboxgl.LngLatBounds(); //the area that will be displayed on the map
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      //we pass the options right on the popup
  ...
  map.fitBounds(bounds, {
    //Tell map to fit our location based on coordinates we specified
    padding: {
      //biar zoom nya ga terlalu in, jd di style
      top: 200,
      //all in pixel(px)
      bottom: 150,
      left: 200,
...
var showAlert = function showAlert(type, msg) {
  //Alert klo usr salah kasi msuk pass
  hideAlert();
        ...
        switch (_context.prev = _context.next) {
          case 0:
            ...
            return (0, _axios.default)({
              //Send a data using http request using axios
              method: 'POST',
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              //this is the data of our JSON response from controllers
              (0, _alert.showAlert)('success', 'Logged in successfully'); //tp harus di klik dulu baru reload sendiri :(

              window.setTimeout(function () {
                //this is how we tell browser to reload the page automatically
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 7:
            ...
            (0, _alert.showAlert)('error', 'Logout failed. Please try again');
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? 'http://127.0.0.1:3000/api/v1/users/updateMyPassword' : 'http://127.0.0.1:3000/api/v1/users/updateMe'; //dari userController
          ...
          case 8:
            _context.prev = 8;
            _context.t0 = _context["catch"](0);
            (0, _alert.showAlert)('error', _context.t0.response.data.message); //the err here is automatically built-in created in the try block when something goes wrong
...
var stripe = Stripe('pk_test_51I4DnQD2A9uBKNptzLZUt7nYMBaPyctGxrQQpjHDxFmgwsei2A4O8vGMUu0uHSV8FOWVeurFLuv5TCa1M1RupFdF00QEw1jnFl');
        ...
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            return stripe.redirectToCheckout({
              sessionId: session.data.session.id //Remember how the session was inside of data 
...
if (mapBox) {
  //if in the page there is an id called map...
  var locations = JSON.parse(mapBox.dataset.locations); //dataset adalah cara utk mengambil data dari backend menggunakan java sript
...
if (logOutBtn) logOutBtn.addEventListener('click', _login.logout); // document.querySelector('.nav__el--logout').addEventListener('click', e => {
//     console.log('ccccc');
// })
...
if (userDataForm) userDataForm.addEventListener('submit', function (e) {
  ...
  var form = new FormData(); //ini built-in multipart form data dari js karena kita mau upload image

  form.append('name', document.getElementById('name').value);
  form.append('email', document.getElementById('email').value);
  form.append('photo', document.getElementById('photo').files[0]); //files is an array, so bcoz we only upload one photo, .files[0]
